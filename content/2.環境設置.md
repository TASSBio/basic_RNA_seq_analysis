...
儘管 `conda install` 指令使用方便，但在需要一次安裝數十個，甚至數百個套件的情況下，效率可能受到挑戰。為了提高效率，我們可以運用 YAML 文件，其中紀錄了套件名稱、版本和通道，以便進行批次安裝。透過 YAML 文件，我們能夠建立一個一致的環境配置。這不僅有助於確保不同環境中的一致性，還能夠有效地進行大規模的套件管理。


建立空白 YAML 文件，並且用文字編輯器開啟
```bash
nano my_package.yaml
```

在 YAML 文件中列舉所有需要安裝的套件，並指定版本和通道。編輯完成後存檔。
```yaml
   name: my_env
   channels:
     - defaults
   dependencies:
     ...
     - numpy=1.21.2
     - matplotlib=3.4.3
     - other_package=version
```
*YAML 格式對於縮排的使用較為嚴謹*
   
使用 conda env update 指令根據 YAML 文件進行批次安裝。這樣的方式不僅節省了手動輸入每個套件的時間，還確保了環境的一致性。
```bash
conda env update --file my_package.yaml
```
現在，你可以起身吃個飯、喝個水、走一走。等待安裝完成。

確認安裝是否成功，並檢查環境中的套件及其版本。
```bash
conda list
```

*小技巧*:
我們可以使用 `conda env create` 指令在創建環境時根據 YAML 文件安裝需要的套件。 
```bash
conda env create --name my_env2 --file my_package.yaml
```
